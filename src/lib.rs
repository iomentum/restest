...
///
/// `url` can be either a single value that implements [`ToString`] or an
/// array of different values (of possibly different types) that all implement
/// [`ToString`]. They will all be concatenated and separated by `/` at runtime.
///
/// ## Specifying the request input and output body
///
/// `input` must be an expression of any type that implements [`Serialize`].
///
/// `output` must be a pattern of any type that implements [`DeserializeOwned`].
...
macro_rules! assert_api {
    ...
    (
        $method:ident [ $( $url_segment:expr ),* $(,)? ],
        $input:expr => $output:pat $(,)?
    ) => {
        let url = vec![ $( Box::new($url_segment) as _, )* ];
...
pub async fn request_and_deserialize<I, O>(
    method: Method,
    url_segments: Vec<Box<dyn ToString>>,
    port: u16,
    input: &I,
) -> O
where
    ...
    O: DeserializeOwned,
{
    let url = create_request_url(port, url_segments);
...
fn create_request_url(port: u16, url_segments: Vec<Box<dyn ToString>>) -> String {
    let mut buff = format!("{}:{}", LOCALHOST_ADDRESS, port);

    for segment in url_segments {
        let segment = segment.to_string();

        if !segment.starts_with('/') {
            // Some people may want to add leading slashes. Let's allow them to.
            buff.push('/');
        }
